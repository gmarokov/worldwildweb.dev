...
# Introduction
Hangfire simplifies performing fire-and-forget, delayed, and recurring tasks within ASP.NET applications. 
It eliminates the need for Windows Services or separate processes and is backed by persistent storage. 
Best of all, it's open and free for commercial use.
...
In this post, we are about to create new ASP.NET Core MVC application, with configured database, then install 
and configure Hangfire to run in the background for us. So let`s get started!
